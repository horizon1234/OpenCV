...
亮度从小到大：
0 (黑)  <  64 (深灰)  <  128 (中灰)  <  192 (浅灰)  <  255 (白)

注：使用 ⬛/⬜ 避免在深色主题下把“黑白方块”看反。
```
...
```
  ...
  关键区分（OpenCV 常见两套写法）：
  - 几何坐标/绘图坐标： (x, y) = (列, 行)
  - 矩阵索引/Mat 访问： (row, col) = (行, 列)

  因此二者是同一个像素的两种写法：
  (x, y) = (2, 1)  ⇔  (row, col) = (1, 2)
  代码里应写：mat.at<uchar>(y, x)  // 即 mat.at<uchar>(1, 2)
```

  #### 0.5.1 几何坐标有什么意义？

  几何坐标不是“多余的一套写法”，它的意义是：**用来描述图像平面上的位置与形状**。

  ```
  把图像当成一张平面：

    几何世界（视觉/绘图）              矩阵世界（存储/访问）
    ───────────────────              ───────────────────
    点在什么位置？(x, y)               数据在第几行第几列？(row, col)
    框有多宽多高？(x, y, w, h)         内存里怎么取值？at<...>(row, col)
    线从哪到哪？Point(x1,y1)->Point(x2,y2)  循环通常是 for(row) for(col)
  ```

  以 1920×1080 图像为例：

  - 宽=1920（列范围 0~1919），高=1080（行范围 0~1079）
  - 右下角最后一个像素：
    - 几何坐标：`(x, y) = (1919, 1079)`
    - 矩阵索引：`(row, col) = (1079, 1919)`
  - 是同一个点，只是表达顺序不同：`x = col`，`y = row`

  为什么工程里必须保留“几何坐标”这套表达？

  1. **和人类直觉一致**：
    说“向右 100 像素、向下 20 像素”，天然是 `(x, y)`。

  2. **和 OpenCV 绘图 API 一致**：
    如 `cv::Point(x, y)`、`cv::line`、`cv::circle`、`cv::rectangle` 都按几何坐标。

  3. **和交互输入一致**：
    鼠标点击事件返回的位置本质是屏幕平面坐标 `(x, y)`。

  4. **和几何变换公式一致**：
    旋转、平移、投影、相机模型等通常写成 $(x, y)$ 或齐次坐标形式。

  一句话记忆：

  - 需要“描述位置/形状/运动”时，用几何坐标 `(x, y)`。
  - 需要“访问像素数值”时，用矩阵索引 `(row, col)`，即 `mat.at(...)(y, x)`。
...
上表中的对数变换值得单独展开——它和 Gamma 变换都是"暗部增强"类操作，但原理不同。
...
其中 $c$ 为归一化常数，使输出映射到 $[0, 255]$：

$$
c = \frac{255}{\ln(1 + 255)} = \frac{255}{\ln 256} \approx 45.99
$$

这里的“归一化常数”可以理解为一个**缩放系数**：

- `ln(1+I)` 的原始输出范围不是 `0~255`，而是 `0~ln(256)≈5.545`
- 如果不乘 `c`，最大值只有 `5.545`，图像会几乎全黑
- 乘上 `c` 的目的，是把最大输入 `I=255` 精确拉到输出 `I'=255`

推导思路（只用一个条件就能解出）：

$$
	ext{要求 } I=255 \Rightarrow I'=255
$$

$$
255 = c\cdot\ln(1+255)=c\cdot\ln 256
\Rightarrow
c=\frac{255}{\ln 256}
$$
...
| 情况 | 公式 | 当 $I=255$ 时输出 | 结果 |
|------|------|-------------------|------|
| 不做归一化 | $I'=\ln(1+I)$ | $\ln 256\approx 5.545$ | 动态范围严重不足 |
| 做归一化 | $I'=\frac{255}{\ln 256}\ln(1+I)$ | $255$ | 完整映射到 8-bit |

#### 映射曲线（用表格看）

先看“输入值 → 输出值”的关键点：

| 输入 $I$ | $\ln(1+I)$ | 输出 $I'=45.99\cdot\ln(1+I)$ | 相对恒等线 $I'=I$ |
|:-------:|:-----------:|:-----------------------------:|:------------------:|
| 0 | 0.000 | 0 | 相同 |
| 16 | 2.833 | 130 | 明显高于恒等线（提亮暗部） |
| 32 | 3.497 | 161 | 明显高于恒等线 |
| 64 | 4.174 | 192 | 高于恒等线 |
| 96 | 4.575 | 210 | 高于恒等线 |
| 128 | 4.860 | 224 | 高于恒等线 |
| 160 | 5.081 | 234 | 略高于恒等线 |
| 192 | 5.263 | 242 | 接近上限 |
| 224 | 5.416 | 249 | 接近上限 |
| 255 | 5.545 | 255 | 与终点重合 |
...
| 输入区间 | $\Delta I$ | 输出增量 $\Delta I'$ | 平均斜率近似 $\Delta I'/\Delta I$ | 解释 |
|:-------:|:----------:|:--------------------:|:----------------------------------:|------|
| 0 → 16 | 16 | 130 | 8.13 | 暗部拉伸极强 |
| 16 → 32 | 16 | 31 | 1.94 | 仍在明显提亮 |
| 32 → 64 | 32 | 31 | 0.97 | 逐步接近线性 |
| 64 → 128 | 64 | 32 | 0.50 | 中间调开始压缩 |
| 128 → 192 | 64 | 18 | 0.28 | 亮部压缩增强 |
| 192 → 255 | 63 | 13 | 0.21 | 高亮区趋于饱和 |
...
下图给出和 `01_weber_fechner` 同风格的对数变换曲线：

![对数变换曲线（暗部拉伸、亮部压缩）](images/13_log_transform_curve.png)

#### 数值示例

| 输入 I | $\ln(1+I)$ | 输出 $I' = 45.99 \times \ln(1+I)$ | 变化趋势 |
|:------:|:----------:|:----------------------------------:|:-------:|
| 0 | 0.000 | **0** | — |
| 10 | 2.398 | **110** | 暗部剧烈拉伸 ↑↑↑ |
| 30 | 3.434 | **158** | 暗部继续拉伸 ↑↑ |
| 50 | 3.932 | **181** | 增速放缓 ↑ |
| 100 | 4.615 | **212** | 中间调压缩 → |
| 150 | 5.017 | **231** | 亮部压缩明显 → |
| 200 | 5.303 | **244** | 亮部几乎饱和 |
| 255 | 5.545 | **255** | 映射到最大值 |

> 注意：输入 0→10 只变了 10 个灰度级，输出却从 0 跳到 110（增量 110）；而输入 200→255 变了 55 级，输出仅从 244 到 255（增量 11）。**暗部拉伸力度是亮部的 10 倍！**

#### 对数变换 vs Gamma 变换对比

| 维度 | 对数变换 $c \cdot \ln(1+I)$ | Gamma 变换 $(I/255)^\gamma$ |
|:----:|:-------------------------:|:-------------------------:|
| **暗部增强** | ⚡ 极强（ln 在 0 附近斜率→∞） | ✅ 可调（γ 越小越强） |
| **参数** | 仅 $c$（通常自动算） | γ 值（0.1~5.0 灵活调） |
| **可调性** | ⭐ 低（只能"开/关"） | ⭐⭐⭐ 高（γ 连续可调） |
| **典型用途** | 频谱图、FFT 幅度显示 | 显示器校正、摄影调色 |
| **亮部处理** | 强烈压缩（高光细节丢失多） | 温和压缩（可控） |
| **工业标准** | 否 | ✅ sRGB / Rec.709 |
...
实际应用场景对比：
  ...
  ┌─────────────────────────┐      ┌─────────────────────────┐
  │ ● FFT 频谱可视化         │      │ ● 照片亮度调整           │
  │   （动态范围极大，需压缩） │      │ ● 显示器 Gamma 校正     │
  │ ● 天文图像（星光差异巨大）│      │ ● 视频编码 / sRGB       │
  │ ● 医学影像对数增强       │      │ ● 游戏画面 HDR→SDR 映射 │
  └─────────────────────────┘      └─────────────────────────┘

  共同点：都是"暗部增强、亮部压缩"
  关键区别：ln 力度更猛且不可调，Gamma 更温和且灵活
```
...
$$I' = I^{1/2.2}, \quad L = I'^{2.2} = I^{1.0} \quad (\text{恢复线性})$$
...
### 9.1 深入理解：γ=0.45 再 γ=2.2 不就等于 γ=1 吗？为什么说暗部精度提高了？

初学者常见的困惑：先做 $\gamma = 0.45$ 编码，显示器再做 $\gamma = 2.2$ 解码，最终亮度还原了——那跟**不编码、不解码（全程线性）**有什么区别？

**区别在于中间的 8-bit 量化（取整）这一步。** 如果精度无限（比如用 float 存储），两条路径确实完全一样。但 8-bit 只有 256 个整数格子可用，取整是不可逆的信息损失，而 Gamma 编码决定了这 256 个格子如何分配。
...
$$L = 0.010,\quad 0.012,\quad 0.014,\quad 0.016,\quad 0.018$$

这 5 个值很接近，但人眼在暗部**能分辨**它们（比如暗巷里的墙壁纹理）。
...
```
步骤 1：相机线性采集 → 直接 ×255 存入 8-bit 文件

  L=0.010 → 0.010 × 255 = 2.55  → 取整 = 3
  L=0.012 → 0.012 × 255 = 3.06  → 取整 = 3    ← 和上一个一样了！
  L=0.014 → 0.014 × 255 = 3.57  → 取整 = 4
  L=0.016 → 0.016 × 255 = 4.08  → 取整 = 4    ← 又重复了！
  L=0.018 → 0.018 × 255 = 4.59  → 取整 = 5

  存储的值：3, 3, 4, 4, 5   ← 5 个不同亮度只剩 3 个不同的值！

步骤 2：显示器线性显示（不做 γ=2.2）

  存储值 3 → 显示亮度 = 3/255 = 0.01176
  存储值 3 → 显示亮度 = 3/255 = 0.01176    ← 原本 0.012 的细节丢了
  存储值 4 → 显示亮度 = 4/255 = 0.01569
  存储值 4 → 显示亮度 = 4/255 = 0.01569    ← 原本 0.016 的细节丢了
  存储值 5 → 显示亮度 = 5/255 = 0.01961
```

**结果**：5 个像素变成了只有 3 种亮度。暗巷里的墙壁纹理 → **色带（条纹状的跳变）**。

---

#### 路径 B：γ=0.45 编码 → γ=2.2 解码

```
步骤 1：相机线性采集 → 做 γ=0.45 编码 → 存入 8-bit 文件

  L=0.010 → 0.010^0.45 × 255 = 18.05 → 取整 = 18
  L=0.012 → 0.012^0.45 × 255 = 19.55 → 取整 = 20
  L=0.014 → 0.014^0.45 × 255 = 20.90 → 取整 = 21
  L=0.016 → 0.016^0.45 × 255 = 22.14 → 取整 = 22
  L=0.018 → 0.018^0.45 × 255 = 23.28 → 取整 = 23

  存储的值：18, 20, 21, 22, 23  ← 5 个不同亮度保留了 5 个不同的值！

步骤 2：显示器做 γ=2.2 解码

  存储值 18 → (18/255)^2.2 = 0.00974  ≈ 0.010 ✓
  存储值 20 → (20/255)^2.2 = 0.01226  ≈ 0.012 ✓
  存储值 21 → (21/255)^2.2 = 0.01371  ≈ 0.014 ✓（差一点点）
  存储值 22 → (22/255)^2.2 = 0.01527  ≈ 0.016 ✓（差一点点）
  存储值 23 → (23/255)^2.2 = 0.01695  ≈ 0.018 ✓（差一点点）
```

**结果**：5 个像素保留了 5 种不同亮度，都还原得很接近原始值。墙壁纹理 → **平滑过渡，没有色带**。
...
```
                         取整（量化）
                             ↓
路径A:  0.010  0.012  0.014  0.016  0.018   （5个不同的值）
        ×255
         ↓      ↓      ↓      ↓      ↓
        2.55   3.06   3.57   4.08   4.59
        ┃      ┃      ┃      ┃      ┃
        ↓      ↓      ↓      ↓      ↓      取整
        3      3      4      4      5       ← 只剩 3 种，丢了 2 个
        ┃      ┃      ┃      ┃      ┃
        显示器线性输出
        ↓      ↓      ↓      ↓      ↓
       0.012  0.012  0.016  0.016  0.020    ← 暗部细节没了


路径B:  0.010  0.012  0.014  0.016  0.018   （同样5个值）
        ^0.45 × 255
         ↓      ↓      ↓      ↓      ↓
       18.05  19.55  20.90  22.14  23.28
        ┃      ┃      ┃      ┃      ┃
        ↓      ↓      ↓      ↓      ↓      取整
        18     20     21     22     23      ← 5 种全保留了！
        ┃      ┃      ┃      ┃      ┃
        显示器 ^2.2 解码
        ↓      ↓      ↓      ↓      ↓
       0.010  0.012  0.014  0.015  0.017    ← 全部还原（有微小误差）
```
...
因为 0.010~0.018 这个范围乘以 255 之后只对应 2.55~4.59，**这段区间里只有 3 个整数（3、4、5）**。5 个不同的值被挤进 3 个格子里，必然有 2 个被合并。

而 γ=0.45 编码后，同样的范围被拉伸到 18~23，**这段区间里有 6 个整数**，5 个值都能找到自己的格子。
...
```
              编码              量化（瓶颈！）           解码
真实亮度 ──→ γ=0.45 ──→ 连续值 ──→ 取整为0~255整数 ──→ γ=2.2 ──→ 你看到的亮度
                              ↑
                    这一步只有 256 个格子可用！
                    编码决定了这 256 个格子
                    怎么分给暗部和亮部
...
| 维度 | 路径 A（全程线性） | 路径 B（γ=0.45 编码 + γ=2.2 解码） |
|------|-------------------|-----------------------------------|
| **最终亮度** | 还原到真实值 | 同样还原到真实值（近似） |
| **暗部占多少整数格子** | 约 26 级（0~255 中的 0~25） | 约 80 级（0~255 中的 0~79） |
| **暗部精度** | 低——相邻暗值被合并成同一个整数 | 高——相邻暗值各自有独立整数 |
| **暗部显示效果** | 色带（条纹状跳变） | 平滑过渡 |
| **亮部精度** | 高（但人眼看不出差别） | 低（但人眼不需要这么多） |

> **一句话总结**：γ=0.45 和 γ=2.2 确实互相抵消还原了亮度，但**精度的提升发生在中间的 8-bit 量化这一步**——编码让暗部占了更多的整数格子，取整损失更小，所以暗部过渡更平滑、没有色带。显示器的 γ=2.2 解码时已经是连续的模拟信号（电压），不再受 256 级的限制。
