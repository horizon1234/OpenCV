...
先记住这一句总纲，后面整篇都会围着它展开：

**显示器最终通常还是按 `RGB/BGR` 发光来显示；转到 `HSV`、`YCrCb`、`Lab` 等空间，主要是为了让颜色处理更顺手。**

比如：

- 想调饱和度，常去 `HSV`
- 想保色提亮，常去 `YCrCb` 或 `Lab`
- 想按颜色类别分割，常去 `HSV`

所以很多 OpenCV 流程本质上都是：

$$
BGR/RGB \rightarrow 更适合任务的色彩空间 \rightarrow 处理 \rightarrow BGR/RGB \rightarrow 显示
$$
...
这里有一个非常常见、也非常值得专门说清楚的疑问：

> 既然 OpenCV 里的 `H` 只有 `0~179` 这 180 种离散取值，那色相环是不是就只有 180 个点？为什么图里这个环还有厚度？`H=100` 难道不会对应很多颜色吗？

答案是：

1. 从“纯数学定义”上讲，`H` 的确只负责描述色相环上的角度位置，所以如果只看 `H`，它更像是环上的一个角度点，或者一个离散角度桶。
2. 图里的“厚度”不是说 `H` 还有第二个维度，而只是为了让图更容易看清楚、能把不同角度的颜色连续画出来。
3. 如果一定要把这张图理解得更严格一点，可以把它看成“固定较高 `S` 和较高 `V` 时的色相环带”。这时角度主要表示 `H`，厚度只是可视化带宽，不是在增加新的色相值。

也就是说：

**色相环的关键是“角度”，不是“厚度”。厚度只是示意图为了可读性故意画出来的。**

进一步说，`H=100` 确实不会只对应一个最终颜色。

因为一个完整的 HSV 颜色必须同时看：

1. `H`：你在色相环上的哪个角度区段
2. `S`：你离“灰色中心”有多远，也就是鲜不鲜艳
3. `V`：你整体有多亮
...
这三个颜色的 `H` 都一样，但它们看起来会很不一样。

因此更准确的理解是：

- `H=100` 对应的是“某一类偏青蓝的色相方向”
- 不是“唯一一个最终颜色”

如果你只是想给 `H=100` 找一个最有代表性的可视化颜色，常见做法是人为固定：

- `S = 255`
- `V = 255`

然后把 `HSV = (100,255,255)` 转回 `BGR` 显示。

这时你看到的是：

**“H=100 在最高饱和度、最高明度条件下的代表色”**

而不是说 `H=100` 在任何情况下都只能是这个颜色。
...
如果上面这几句还是偏抽象，先直接看下面这张对比图：

![Lab 感知均匀性的对比示意图](lab_perceptual_uniformity.svg)

这张图想表达的不是“Lab 完全完美”，而是一个更实用的结论：

- 在 `BGR` 里，同样大小的数字改动，落到不同底色上，主观变化可能差很多。
- 在 `Lab` 里，如果你固定 `a/b`、只改 `L`，或者做同样大小的 `a/b` 调整，主观上更容易得到“差不多程度的变化”。

可以先抓两个最具体的对比例子：

### 对比例子 1：同样都是“+20”，在 BGR 里未必是同样程度的变化

比如你在 `BGR` 里做下面这些改动：

1. 灰色从 `(60,60,60)` 变成 `(80,80,80)`，你主要感觉是“亮了一点”。
2. 暖色从 `(60,120,200)` 变成 `(80,120,200)`，你感受到的往往不只是亮暗变化，还带上了一点偏色变化。
3. 高亮灰从 `(210,210,210)` 变成 `(230,230,230)`，虽然数值还是 `+20`，但主观上有时又会觉得没前面那么明显。

这就说明：

**在 BGR 里，同样的数值步长，不一定对应同样的视觉步长。**

### 对比例子 2：在 Lab 里固定 a/b，只改 L，更像“同色系提亮”

如果你换到 `Lab`，并且保持 `a`、`b` 不变，只把 `L` 做相同幅度的增加，比如都做：
...
1. 主要变化是同一类颜色变亮了。
2. 不容易像直接改 `BGR` 那样顺带把颜色关系带偏。
3. 不同底色下，这种“提亮一步”的主观感觉更容易接近同一个层级。

这就是“更接近感知均匀”的真正含义。

它不是说：

- `Lab` 里的每一个数值变化都绝对完美一致

而是说：

- **Lab 的坐标轴设计，比 BGR 更接近人眼实际感受到的亮暗和颜色差异。**

所以如果你要做下面这些任务，`Lab` 往往就会更顺手：

1. 计算颜色差异
2. 做感知更均匀的聚类
3. 只调整明度，不希望颜色结构被明显带偏
...
- `BGR`：适合做快速、粗粒度的滤镜风格
- `Lab`：更适合做更接近人眼感受的暖冷调整
- `HSV`：如果你本质上是在改整体色相风格，也可以用

这里需要特别澄清一下：

如果你的目标只是做一个**简单、快速、成本低的暖色滤镜**，那 `BGR` 当然可以用，比如：
...
这样得到的“暖”更接近感知上的暖色变化，而不是简单的 BGR 通道失衡。

所以更准确的结论应该是：

- **简单滤镜、快速粗调：`BGR` 可以用**
- **更讲究感知一致性和可控性：`Lab` 往往更好**
...
这个例子保留下来，是因为它很符合很多初学者最先会写的“滤镜式处理”：
...
如果你更在意“人眼感觉上的偏暖，而且希望变化更稳”，那么用 `Lab` 去调 `a/b` 会更合理。

下面这张图可以先把两种思路的区别直观看出来：

![BGR 变暖与 Lab 变暖的对比示意图](warm_adjust_bgr_vs_lab.svg)

你可以先把它理解成：

- `BGR` 更像“直接拉通道做滤镜”
- `Lab` 更像“沿人眼感受到的偏红 / 偏黄方向轻推”
...
补充一句：除了“只提亮 L”，`Lab` 也很适合做更感知化的暖冷调整。
...
这类改法通常比直接在 `BGR` 里拉通道，更接近“人眼感觉图片变暖了”这件事。

如果要把这个思路写成一个最小可运行例子，可以像下面这样做：
...
cv::add(channels[1], cv::Scalar(8), channels[1]);  // a 略增，更偏红
cv::add(channels[2], cv::Scalar(10), channels[2]); // b 略增，更偏黄

cv::merge(channels, lab);
cv::cvtColor(lab, result, cv::COLOR_Lab2BGR);
```

这个例子和 13.1 的 `BGR` 版本形成了一个很好的对照：

1. `BGR` 版本更像“快速滤镜”，实现短、见效快。
2. `Lab` 版本更像“感知方向调色”，更适合认真做暖冷氛围调整。
3. 如果你只是想快速套一个暖色风格，`BGR` 完全可以用。
4. 如果你想让“变暖”这件事本身更可控、更接近人眼感受，`Lab` 往往更合理。
